...
You will add `"api-compute"` instruction that sends the string with an api call to mathjs.org. A helper function is already there to create the url you will send to that server.
...
### Task 1: Complete the NumericProcessor Class
...
- Write a `run_computations` method that does this:
  - For each entry in `self.computations_list`, pass the entry to a `run_single_computation` method:
    - This method looks at the computation name `computation['operation']` and calls a corresponding method and passing the values `computation['values']`.
...
- The code to load computations from a file and to make an API call to the mathjs is given to you along with an example code at the bottom of the file. You don't need to make any changes to `load_computations_list_from_file`.

Hint:You can start with the `add` method. It should take a list of two numbers and add them. Next, make it possible for computation to refer to the previous result as `ANS`. Tip: using an attribute on the class would be a good way to do this.
You can then test this using the `example_add.json` file.

```
{
    "computations": [
    { "operation": "add", "values": ["2.54", "1.89"] },
    { "operation": "display", "values": ["ANS"] }
  ]
}
```

Once you are confident that the `add` method works, you can move on to the other methods.
...
- You will then write a new `send_to_api` method on the class
  ...
  - Return the number.
- Update the `run_single_computation` method to look for `"api-compute"`. If that is the operation, send the first value to this new `send_to_api` method and store the result.

The `NumericProcessor` should now be able to run the `example_api.json` file.

### Task 2: Adding a mode that gathers statistics

Use **class inheritance** to extend the NumericProcessor behavior and add the ability to run in a different mode that gathers statistics.
...
- Create a `run_single_computation` method that takes a computation.
  - This method overrides the parent class `run_single_computation`.
  - Use the `count_operations` dictionary to keep track of how many times each type of operation is seen.
  - Use `super().run_single_computation(computation)` so that it still calls the parent class method.
...
You can now change the test code at the bottom of the file. You can now create an instance of `OperationCounterNumericProcessor` and use it just like `NumericProcessor`. At the end, you can call `show_statistics` on the instance and see the counts.
